...
> 最后更新：2026-09-14 | 当前版本：v2.6.0

## 当前设计思路

- 首次引导按输入习惯、候选外观、可选语音下载、AI 与数据、启用试用五步组织；纯打字不依赖语音模型。设置独立提供候选外观、历史评估和隐私存储入口。
- AI 触发方式统一为关闭、专用快捷键、每次语音，运行与测试共用模型/超时/提示词配置。
- 音频素材默认不留存，可选最近 500 条或最多 5 GB；两者均有容量保护。支持原始识别对照、试听、人工参考稿和 CER；多模型批量评测后续扩展。

轻语（QEcho）定位为 macOS 上的低打扰语音输入工具。应用优先保持后台常驻，通过全局快捷键、悬浮波形和本地语音转写能力，把 AI 语音输入送到用户正在使用的输入场景里。

## 主要功能

- 系统输入法（实验性）：QEchoIME 作为独立 IMKit 输入法随主 App 分发，输入源菜单显示「轻语输入法」并使用 QEcho App 图标，设置页一键安装到 `~/Library/Input Methods` 并注册启用。打字由 librime 驱动，三方案可切换：五笔·拼音混打（默认）/ 纯五笔 86 / 纯拼音，设置页或输入法下拉菜单均可切换并双侧同步；`` ` `` 前缀拼音反查五笔编码、内联组字 + 自绘候选窗（横/竖排、字体字号、浅深两套配色、圆角间距、序号编码显隐均可在设置页定制并实时预览）。候选快捷键：数字选字、空格首选、分号次选、引号三选（纯五笔）、`-`/`=` 与 `,`/`。` 翻页、Tab 轮换、回车上原始编码；有汉字候选时 Shift 上屏当前英文编码并保持中文，无汉字候选时才切换英文。动态词频（Rime 用户词库）可在设置页开关；候选采用率学习与该开关解耦，始终在本机聚合记录候选曝光、采用次数/采用率和选择位置，不上传，关闭动态排序也不会丢失采用习惯。输入法下拉菜单提供打开轻语设置、方案切换、中英/全角开关。选中轻语输入法时，语音文本经 CFMessagePort → `IMKTextInput.insertText` 走系统输入法通道上屏（失败自动回退 CGEvent / 剪贴板），插入前自动落掉进行中的组字。语音输入（FN 热键 + 录音 + 转写）仍住在主 App，因此输入法进程在焦点进入输入框时会检查主 App 是否在跑，不在就带 `--background-launch` 静默拉起（不抢焦点、不显示窗口，15 秒节流；用户主动退出主 App 后本次开机不再拉起）——保证「只选中输入法、没开过设置窗」时按 FN 同样能唤起语音输入。词典预编译随包分发（含纯五笔 prism），用户词典、设置与采用率统计在 `~/Library/Application Support/QEchoIME/`。
...
- `2.6.0`：重做首次引导与设置结构。引导覆盖输入方案、候选排序、候选样式、可选语音模型下载、AI 触发与音频留存，纯打字用户不下载语音模型也能配置完成；设置拆为输入与词库、候选窗外观、语音输入、AI 文本处理、历史与评估、隐私与存储六组。新增本地语音评估素材库（最近 500 条或最多 5 GB，默认关闭，单次音频上限 128 MB），原始识别、规则纠错与最终输出分别留存，支持试听、人工参考稿与字符错误率。AI 触发统一为关闭 / 仅专用快捷键 / 每次语音三档，录音与设置测试共用同一份服务、模型、超时与提示词解析。修设置窗口最小尺寸与侧栏边距导致的裁切，修 Debug 安装器误把 `QechoIME.app` 当主 App 产物。
- `2.5.0-rc7`：修掉静音检测单测对真实时钟的依赖——`SilenceDetector` 改为可注入时钟，测试用假时钟推进时间，高负载机器上不再假阳性失败。
- `2.5.0-rc6`：修语音识别慢（停顿或松键后要再等好几秒）。四处叠加：ONNX 线程数取逻辑核数把能效核也拉进线程池且开着自旋（本机 6 秒语音 479ms → 151ms）、模型 5 分钟空闲就卸载而 fp32 重载要 12~19 秒、启动预热用 `.utility` 把 ONNX 线程池 QoS 钉在低优先级（慢 1.5~2 倍）、加载后首次推理的惰性分配没预热。同时改掉松键后「尾缓冲 + 等分段转写」串行相加、智能分段白白重采样整段录音、音频缓冲平方复杂度拼接。改用官方 int8 量化模型（241MB，15 句测试与 fp32 逐字一致，推理快一倍），与历史 fp32 双变体共存、加载优先加速版，老用户可在设置页一键升级并回收旧模型磁盘。
- `2.5.0-rc5`：支持本地 Ollama 上的 Gemma 4（`gemma4:e4b-it-qat` / `e2b-it-qat`）做语音 AI 校正，进入推荐模型与可下载模型库；所有 Ollama 原生请求统一关思考（实测 e4b 修正一句话 20.9s → 1.6s）；Ollama 默认超时 5s → 60s 并迁移历史配置（旧值让本地模型必然超时）；按 AI 快捷键录音时并行预热本地模型，把 6~8 秒冷加载藏进录音时间里。
- `2.5.0-rc4`：修「选中轻语输入法后按 FN 唤不起语音输入，必须先打开一次设置窗口」——FN 监听住在主 App 且装配点挂在主窗口 onAppear，只选输入法时主 App 压根没在跑。现热键装配移到 App 启动阶段（无窗口也注册），输入法进程在焦点进入输入框时静默拉起主 App（`--background-launch`，不抢焦点、不显示窗口，带 15 秒节流与「用户主动退出则本次开机不再拉起」的抑制标记）；同时补齐缺辅助功能权限时的菜单栏警示与一键授权入口，授权后自动重注册热键。
- `2.5.0-rc3`：修智能分段丢字（过短片段被取出后直接丢弃，改为结转到下一段）、静音检测把整句从中间切碎（峰值只增不减导致相对阈值虚高，加入峰值衰减并区分绝对/相对静音时长）、语音输入默认快捷键与 FN 文案不符、主快捷键非 FN 时 FN+Control 完全失效。
- `2.5.0-rc2`：移除源码零引用的 GRDB.swift 与唯一 CocoaPods 集成，主 App 不再携带或链接 GRDB，测试和 Release 构建可直接使用 Xcode project。
- `2.5.0-rc1`：Shift 智能临时英文——有汉字候选时提交当前英文编码但保持中文，无汉字候选时才切换英文；新增始终开启的本地候选采用率学习，记录候选曝光、采用次数/采用率与位置，并与动态词频排序开关解耦。
...
- `2.4.1-rc1`：修输入源菜单「轻语输入法」占据超高纯色区块的显示异常——模板图 PDF 画布从 768pt 改为 16pt（对齐系统 CVIconTemplate），菜单恢复正常一行图标。
- `2.4.0`：定版。轻语从语音输入工具升级为完整系统输入法（QEchoIME：五笔·拼音混打 / 自绘候选窗全定制 + 6 套预设 / 候选个数 5–9 / 输入法菜单）；设置窗成为主窗口，左侧 5 组 tab，语音输入拆 3 子 tab，新增帮助 tab；语音输入延续（FN 全局唤起 / AI 优化 / Power Mode）。build 43，前后端与 STT API 版本一致。
- `2.4.0-rc13`：移除「数据与其他」tab 的「关于」入口（设置已是主窗口，关于窗内容此 tab 已具备）。
- `2.4.0-rc12`：「语音输入」tab 拆为 3 个二级子 tab（顶部 segmented）：设置（快捷键/触发/识别语言/分段/悬浮条/插入/权限/默认语言/测试框）、统计与历史、Power Mode；Power Mode 从「AI 与模型」tab 移入语音输入子 tab。
- `2.4.0-rc11`：设置窗口成为主窗口（轻语已是输入法，打开 App/点 Dock 直接进设置窗左侧 tab，去掉旧主窗+齿轮 sheet）；测试输入框/语音输入统计/语音输入历史并入「语音输入」tab 各项设置之后；Cmd+, 与输入法菜单「打开轻语设置」改为把主窗口拉到前面。删除 VoiceInputView。
- `2.4.0-rc10`：修点击 Dock 图标出现两个主窗口（reopen 始终返回 false，由 bringToFront 找回已有窗口）；主窗口统计区加「语音输入统计」标题、历史区改为「语音输入历史」，明确数据来自语音输入。
- `2.4.0-rc9`：设置窗口结构调整。移除「常用」tab（删 Dock 隐藏开关，仅剩的隐私说明各 tab 顶部已有）；「默认语言」移入「语音输入」tab（只影响语音识别）；「输入法·打字」提为第一个且默认选中。左侧 5 组：输入法·打字/语音输入/AI 与模型/数据与其他/帮助。
- `2.4.0-rc8`：设置窗口新增「帮助」tab（左侧第 6 组），内容全面：输入法快捷键表（含 -/= 与 ,/。翻页）、语音输入指南、安装与排障、常见问题 FAQ；5 语种本地化。
