...
/// 会议记录管理器
@MainActor
class MeetingRecordManager: ObservableObject {
    static let shared = MeetingRecordManager()
    ...
    /// 按时间范围查询记录
    func records(from startDate: Date, to endDate: Date) -> [MeetingRecord] {
    ...
    private func loadRecords() async {
        ...
        let decoded = await Task.detached(priority: .userInitiated) {
            try? JSONDecoder().decode([MeetingRecord].self, from: data)
        }.value
