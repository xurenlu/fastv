...
struct AudioExtractor {
    ...
    static func extractAudio(
        ...
        // MP3 格式：优先尝试使用 ffmpeg（如果可用）
        if format == .mp3 && FFmpegHelper.isAvailable() {
            let durationSeconds = CMTimeGetSeconds(duration)
            try await FFmpegHelper.extractAudio(
                from: videoURL,
                to: outputURL,
                format: format,
        ...
        // M4A 格式可以使用 AVAssetExportSession
            ...
            // MP3（ffmpeg 不可用时）和 WAV 需要使用 AVAssetReader/Writer
            // 如果 format 是 MP3 但 ffmpeg 不可用，实际会导出为 M4A 格式
            // 需要调整输出 URL 的扩展名
