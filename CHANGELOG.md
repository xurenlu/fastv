...
2.6.0-rc1 至 rc5 的全部内容转为正式版发布，本轮主题是「重做首次引导与设置结构」。

### Added

- 首次引导覆盖输入方案、候选排序、候选样式、可选语音模型下载、AI 触发方式与音频留存选择；不下载语音模型也能完成打字配置。
- 设置拆分为输入与词库、候选窗外观、语音输入、AI 文本处理、历史与评估、隐私与存储六组；模型下载不再是完成引导的前提。
- 本地语音评估素材库：最近 500 条或最多 5 GB 保留策略、自动清理、试听、单条删除与清空、人工参考稿与字符错误率。原始识别、规则纠错与最终输出分别留存，默认关闭，单次音频上限 128 MB。
- 候选采用统计可独立关闭，关闭后不影响动态排序与既有词库。

### Fixed

- AI 触发统一为关闭、仅专用快捷键、每次语音三档；录音与设置测试共用同一份服务、模型、超时与提示词解析，AI 返回空时回退到识别原文。
- 语音模型按预期大小校验，可选 int8 或 fp32，下载后验证模型能否真正加载。
- 设置窗口在 NSWindow 层固定 `900×640` 最小尺寸，并自动扩展历史保存的过小窗口，侧栏与右侧内容不再同时被裁切。
- 修正侧栏内边距 modifier 顺序，固定宽度布局下 padding 不再反向溢出裁切左侧图标。
- 修正设置窗口左侧导航的布局约束，图标、文字与选中背景在窗口内保持稳定边距。
- 修正 Debug 安装器按产物名匹配时误选 `QechoIME.app` 的问题：主 App 产物统一为 `QEcho.app`，保留 `Qecho` 可执行文件名，确保图标与主 App 资源随包安装。

### Engineering

- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0，build 64。
- 新增 AI 触发与迁移、音频留存撤销、配额边界、WAV 格式、素材读写删除及 CER 测试。

## [2.6.0-rc5] - 2026-09-14

### Fixed
- 在 NSWindow 层设置 `900×640` 最小尺寸，并自动扩展历史保存的小窗口，避免侧栏和右侧内容同时被裁切。
- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0-rc5，build 63。

## [2.6.0-rc4] - 2026-09-14

### Fixed
- 修正侧栏内边距 modifier 顺序，避免固定宽度布局下 padding 反向溢出并裁切左侧图标。
- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0-rc4，build 62。

## [2.6.0-rc3] - 2026-09-14

### Fixed
- 修正 Debug 安装器按产物名称匹配时误选 `QechoIME.app` 的问题，主 App 产物统一为 `QEcho.app` 并保留 `Qecho` 可执行文件名，确保图标和主 App 资源随包安装。
- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0-rc3，build 61。

## [2.6.0-rc2] - 2026-09-14

### Fixed
- 修正设置窗口左侧导航的布局约束，保证图标、文字与选中背景在窗口内保持稳定边距。
- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0-rc2，build 60。

## [2.6.0-rc1] - 2026-09-13

### Added

- 首次引导新增输入方案、候选排序、候选样式、可选语音模型下载、AI 触发方式与音频留存选择；无需下载语音模型即可完成打字配置。
- 设置拆分输入与词库、候选窗外观、语音输入、AI 文本处理、历史与评估、隐私与存储；模型下载独立于首次引导完成状态。
- 新增本地语音评估素材库，支持最近 500 条或最多 5 GB 保留策略、自动清理、试听、单条删除与清空、人工参考稿及字符错误率；原始识别、规则纠错和最终输出独立保存。留存默认关闭，两种策略均受 5 GB 上限保护，单次音频最多 128 MB。
- 候选采用统计可独立关闭，不修改动态排序或删除已有词库。

### Fixed

- AI 触发统一为关闭、仅专用快捷键、每次语音；录音与设置测试共用有效服务、模型、超时和提示词解析，空 AI 输出回退识别结果。
- 语音模型按预期大小校验，支持选择 int8 或 fp32，并在下载后验证模型能否加载。

### Engineering

- 主 App、输入法、测试 target 与 STT API 版本同步为 2.6.0-rc1，build 59。
- 增加 AI 触发与迁移、音频留存撤销、配额边界、WAV 格式、素材读写删除及 CER 测试。
...
- 修复多显示器下悬浮指示器「有时候完全不显示」：它其实显示了，只是画在了另一块屏上。固定位置（左上/右上/左下/右下/正中下方）原先用 `NSScreen.main` 定位，而这个 API 返回的是「包含当前键盘焦点窗口的屏幕」；轻语常驻后台、平时没有键窗口，于是它基本固定落回主显示器。用户在副屏打字按下快捷键，指示器就被画到了主屏，从用户视角看就是没出现。现改为取鼠标所在的屏幕——跨屏切换几乎总是伴随鼠标移动，且取鼠标位置是同步零成本的，不像 AX 查询那样会给按键路径增加延迟。「跟随光标」模式本来就按锚点选屏，不受影响。

### Engineering

- 新增 `CursorPositionLocator.activeScreen()` 统一屏幕选取，并配套纯函数 `indexOfScreen(containing:in:)` 便于在单显示器机器上单测。`WaveformView.calculateWindowFrame()` 改为调用它。
- `CursorPositionLocatorTests` 新增 5 例：鼠标在主屏/副屏分别命中对应屏幕、主屏左侧负坐标空间的副屏同样命中、点不在任何屏幕内时返回 nil 交由调用方兜底、跟随光标模式把窗口 clamp 在锚点所在副屏内。副屏那一例用的坐标就是报告该问题的机器的实际布局（主屏 2560×1600 + 右侧 4K 缩放到 1920×1080）。
- 版本号 `2.5.0-rc8` → `2.5.0-rc9`，build `56` → `57`；主 App、QechoIME、测试 target、STT API 与 `X-API-Version` 响应头保持一致。

### Known Issues

- 若系统「键盘 → 按 🌐 键来」设为「开始听写」，FN 会同时触发 macOS 自带听写，与语音输入抢麦克风和焦点，表现为时灵时不灵。需用户在系统设置里改为「无操作」，App 侧无法代改。
...
- 修复选中轻语输入法后按 FN 唤不起语音输入，必须先打开一次设置窗口才生效：FN 全局监听住在主 App，而它的装配点挂在主窗口 `ContentView.onAppear` 上——用户只在系统里选中输入法时，系统只拉起 QechoIME 输入法进程，主 App 根本没在跑，自然没人监听 FN；点「打开轻语设置」恰好把主 App 拉了起来，于是造成「开过设置才好使」的错觉。现做两处根治：① 语音输入装配（麦克风/辅助功能权限、FN 热键、配置监听）移到 `AppDelegate.applicationDidFinishLaunching`，主 App 无窗口也注册监听，且不再有原先「延迟 1 秒再注册」的空窗；② 输入法进程在每次焦点进入输入框（`activateServer`）时检查主 App 是否在跑，不在就用 `--background-launch` 静默拉起——不抢焦点、不显示窗口，只起菜单栏与热键。
- 修复缺少辅助功能权限时「按 FN 毫无反应且无从自证」：全局事件监听在未授权时不报错、只是永远收不到事件。现菜单栏图标在闲置且未授权时变为橙色警示图标，托盘菜单置顶「语音输入未就绪：缺少辅助功能权限」一项可直接跳转系统设置授权页；授权成功后自动重新注册全局热键并恢复常规图标，无需重启 App（未授权时注册的监听器不会自己复活）。

### Changed

- 主 App 新增静默启动模式（启动参数 `--background-launch`，或环境变量 `QECHO_BACKGROUND_LAUNCH=1`）：仅由输入法进程使用，启动后强制 `.accessory` 激活策略、主窗口建完即隐、不弹辅助功能授权框，并延迟 3 秒预热语音模型，避免用户第一次按 FN 干等模型加载。用户从托盘「显示轻语」/ Dock / 输入法菜单「打开轻语设置」主动要窗口时立即退出静默态，Dock 图标策略回到用户偏好。
- 用户主动退出主 App（托盘退出 / Cmd+Q）时写入抑制标记，输入法在同一次开机会话内不再自动把它拉回来，重启后自动恢复；主 App 被前台启动时清除该标记。崩溃不写标记，因此崩溃后输入法仍会正常兜底拉起。自动拉起本身有 15 秒节流，主 App 起不来时不会随焦点切换反复重试。

### Engineering

- 新增 `MainAppLaunchPolicy`（主 App 与 QechoIME 共享编译）承载纯决策逻辑：拉起节流、退出抑制与开机会话失效判定、静默启动标记识别；配套 `MainAppLaunchPolicyTests` 9 条用例覆盖上述分支与抑制标记的落盘往返。
- 新增 `QEchoIME/MainAppLauncher`（输入法侧拉起器，同时收编原先散在控制器里的「打开轻语设置」启动代码）、`BackgroundLaunchController`（静默启动态）、`AccessibilityTrustWatcher`（未授权期间 5 秒轮询、授权即停并回调重注册热键）。
- `fastvApp` 中语音输入相关方法改为静态方法，热键装配与配置订阅不再依赖 SwiftUI 视图生命周期与 `@State`。
- 版本号 `2.5.0-rc3` → `2.5.0-rc4`，build `51` → `52`；主 App、QechoIME、测试 target、STT API 与 `X-API-Version` 响应头保持一致。

### Known Issues

- 辅助功能权限仍是语音输入的硬前提：输入法进程本身不监听 FN，未授权时静默启动的主 App 只能靠菜单栏警示提示用户。让输入法在有焦点时直接兜底转发 FN（需实测 IMKit 是否投递 fn 的 `flagsChanged`）留待后续版本验证。

## [2.5.0-rc3] - 2026-08-15

### Fixed

- 修复智能分段丢字：`extractCurrentSegmentWithTiming()` 会无条件清空段落缓冲，而调用方在段落短于 0.75 秒时直接 `return`，导致这段已经取出的音频被永久丢弃（表现为句子中间无故少字）。现改为把过短片段结转到下一段开头一起转写，松键时的末段同样并入结转缓冲，任何录入的音频都不会再被扔掉。
- 修复静音检测把整句从中间切开：说话峰值 `speechPeakLevel` 原本在一段连续语音内只增不减，一次重音（或咳嗽、敲键盘）会长期抬高相对静音阈值（峰值 × 25%），使同一句话后半段的正常音量被判为静音而提前切段，切出的短碎片缺少上下文、识别准确率明显下降。现加入峰值缓慢衰减，并要求相对下降判定的静音持续到 1.5 倍时长才切段（绝对阈值静音仍按原时长立即切）。
- 修复语音输入默认快捷键与界面文案不符：存储默认值是 `⌥V`，而设置页「恢复默认」与 `fn.key.hint` 帮助文案均写 FN，全新安装的用户按住 fn 不会有任何反应。现默认主快捷键统一为 FN 单键；已显式设置过快捷键的用户不受影响。
- 修复主快捷键非 FN 时 FN+Control（语音输入 + AI 校正）完全失效：事件分流只按主快捷键判断是否走 FN 分支，FN 的 `flagsChanged` 事件进不了处理路径。现次快捷键为 FN 时同样进入 FN 分支，且仅按 keyCode `0x3F` 识别，避免方向键等自带 `.function` 标志的普通按键被误吞；同时收紧类型判定，主快捷键不是 FN 时单按 FN 不再误触发纯语音输入。

### Engineering

- 新增 `SilenceDetectorTests`：覆盖峰值衰减后正常音量不再被误判为静音、不衰减时的回归对照、绝对静音仍立即判定、语音恢复重置状态，以及相对静音需更长持续时长才切段。
- 版本号 `2.5.0-rc2` → `2.5.0-rc3`，build `50` → `51`；主 App、QechoIME、测试 target、STT API 与 `X-API-Version` 响应头保持一致。

### Known Issues

- 「二次拼接转写」（`enableBatchRefinementTranscription`）自 v2.0.0-rc5 起仍为关闭状态。它原本用于把多个碎片音频合并后重跑一次 ASR 以提升准确率，关闭后碎片识别结果直接拼接输出。本次修复从源头减少了碎片的产生，是否重新开启这条路径需要另行做速度与准确率的权衡验证。
...
- 新增始终开启的本地候选采用率学习：键盘选字（数字、空格、分号、纯五笔引号）与鼠标点击统一记录“输入方案 + 原始输入码 + 当前页候选 + 选中候选/位置”，聚合候选曝光次数、采用次数和采用率，并区分动态排序开启/关闭时的选择。关闭动态词频只停止 Rime 用户词库参与排序，不再丢失用户采用习惯数据。
- 采用率数据原子写入 `~/Library/Application Support/QechoIME/candidate-adoption-statistics.json`，仅保存在本机、不上传；连续选择合并延迟写盘，候选统计最多保留最近 20,000 组，避免长期使用后文件无界增长。

### Changed

- 重做独立 Shift 行为：中文模式且当前页存在汉字候选时，按 Shift 直接提交当前原始英文编码、清空组字并保持中文模式；只有当前没有汉字候选时才切换到英文模式。Shift 作为组合键使用时不会误触发模式切换。
- 设置页动态词频开关下方新增采用率持续本地学习说明，帮助页与快捷键提示同步新 Shift 语义；简中、英文、日文、韩文、粤语 5 语种完整更新。

### Engineering

- 新增候选采用率聚合、编解码及 Shift 汉字候选判定单测；QechoIME target 纳入共享学习存储并同步更新幂等工程生成脚本。
- 版本号 `2.4.1-rc5` → `2.5.0-rc1`，build `48` → `49`；主 App、QechoIME、测试 target、STT API 与 `X-API-Version` 响应头保持一致。
...
- 修输入源菜单里「轻语输入法」图标比其它输入法明显小一圈的问题。双重原因：rc1 生成的模板 PDF 画布实际是 12×12pt（SVG `width="16"` 被 rsvg 按 16px@96dpi 折算成 12pt），且图形只占画布 ~72%（四周留白过多）。修复：SVG 尺寸单位改为 `16pt`，viewBox 裁掉留白使图形占比 ~88%，重新生成 `QechoIMETemplate.pdf`（MediaBox 已验证为 16×16pt）。升级后需在设置「输入法·打字」点「更新并重新启用」，图标缓存可能需注销重登后刷新。

### Engineering

- 版本号 `2.4.1-rc3` → `2.4.1-rc4`，build `46` → `47`；同步 4 处版本点。

## [2.4.1-rc3] - 2026-07-24

### Changed

- **移除设置窗口右上角「完成」按钮**：设置窗口有标准的窗口关闭按钮，工具栏里额外的「完成」按钮属冗余。一并移除不再使用的 `@Environment(\.dismiss)`。

### Engineering

- 版本号 `2.4.1-rc2` → `2.4.1-rc3`，build `45` → `46`；同步 4 处版本点。
...
- 修输入源菜单里「轻语输入法」占据超高纯色区块的严重显示异常：`tsInputMethodIconFileKey` 指向的模板图 PDF 画布是 768×768pt，输入源菜单按图标固有尺寸渲染，导致撑出一整块被选中高亮铺满。改为 16×16pt 画布（对齐系统 CVIconTemplate 量级），菜单里恢复为正常一行图标。源图 `assets/brand/qecho-ime-menubar-template-16.svg`。

## [2.4.0] - 2026-07-24

轻语从「语音输入工具」升级为**完整的系统输入法**：既能语音输入，也能正常打字（五笔·拼音混打）。本版核心能力汇总（详细迭代见下方 rc1–rc13）：

### 系统输入法（QEchoIME）

- 独立 IMKit 输入法随主 App 分发，一键安装到 `~/Library/Input Methods` 并注册启用；输入源菜单显示「轻语输入法」，使用单色矢量模板图标（随系统明暗反色）。
- **五笔·拼音混打**（librime 驱动）：五笔 86 码与全拼在同一输入串混出候选，不用切模式；另可切纯五笔 86 / 纯拼音方案；`` ` `` 前缀拼音反查五笔编码；Shift 中英切换；动态词频（用户词库）可开关；候选个数 5–9 可选。
- **候选快捷键**：数字选字、空格首选、分号次选、引号三选（纯五笔）、`-`/`,` 上一页、`=`/`。` 下一页、Tab 轮换、回车上原码、Esc 清空。
- **自绘候选窗 + 全面外观定制**：横/竖排、字体字号、7 色配色（浅深两套 + 跟随系统）、圆角/间距、序号与编码提示显隐；6 套一键预设皮肤（经典浅色/夜幕/清新绿/极简黑白/大字护眼/竖排水墨）。
- 输入法下拉菜单（打开设置 / 方案切换 / 中英 / 全角）；语音文本经系统输入法通道上屏（比模拟按键可靠）。

### 主 App 与设置

- **设置窗口即主窗口**：轻语已是输入法，打开 App / 点 Dock 直接进设置。左侧竖向 5 组 tab：输入法·打字（默认）/ 语音输入 / AI 与模型 / 数据与其他 / 帮助。
- **「语音输入」tab 三个二级子 tab**：设置（快捷键/触发/识别语言/分段/悬浮条/插入/权限/默认语言/测试框）、统计与历史、Power Mode。
- **新增「帮助」tab**：输入法快捷键表、语音输入指南、安装排障、常见问题 FAQ，5 语种。
- 移除主窗口 7 套换肤（固定系统默认配色）；推荐应用统一为精致网格；修复点击 Dock 图标出现两个主窗口的问题。

### 语音输入（延续并入 IME 形态）

- 按住 FN 录音（全局快捷键，任何 App 可用），FN+Control 走 AI 优化；三种触发方式；术语包、上下文回改、智能分段、Power Mode 场景感知 prompt。
- 语音转写全部本地完成，不联网；仅 AI 优化时发送转写文本（可关闭）。

### 版本

- 定版 `2.4.0`（build 43）。前端 / 后端 / STT API 版本号一致。

## [2.4.0-rc13] - 2026-07-24

### Removed

- 移除「数据与其他」tab 里的「关于」入口（点开独立关于窗口）：设置窗已是主窗口，关于窗承载的推荐应用、官方支持页等内容在本 tab 已具备，独立关于窗多余。`AboutView` / `AboutWindowOpener` 暂保留代码（无引用），以备将来从 App 菜单接回。

### Engineering

- 版本号 `2.4.0-rc12` → `2.4.0-rc13`，build `42` → `43`；同步 4 处版本点。
